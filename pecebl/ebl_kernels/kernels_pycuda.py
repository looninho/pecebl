...
class PsfDoseGenerator():
    def __init__(self, mat_size, writefield_nm, pixel_nm):
        """Class that holds generation of the kernel for convolution and 

        Args:
            mat_size ([type]): [description]
            writefield_nm ([type]): [description]
            pixel_nm ([type]): [description]
        """        
        self.mat_size = mat_size
        self.wf_nm = writefield_nm
        self.pixel_nm = pixel_nm
        #self.psf2d = None
        #self.dose2d = None
        
    def setgpu(self):
        global kernel_gen_psf
        global kernel_pattern_to_matrix
        
        src_module = r'''
        #include <stdint.h>
        #include <stdio.h>
        #include <math.h>
        __device__ double psf_interp1_toonoisy_tofixit(double r, double sr, uint16_t nr, double *data) {
            double y, a, b;
            y = 0.0;
            int i_r;
            i_r = r / sr;
            if (i_r < (nr - 1)) {
                a = (data[i_r+1] - data[i_r]) / sr;
                b = data[i_r+1] - a * i_r * sr;
                y = (r == sr*i_r) ? data[i_r] : a * (r-i_r*sr) + b;
            }
            else {
                y = (i_r == nr - 1) ? data[i_r] : 0.0;
            }
            return y;
        }
        
        __device__ double psf_interp1(double r, double sr, uint16_t nr, double *data) {
            double r_i = r / sr;
            double y, a;
            int i_lo = (int) r_i;
            int i_hi = (r_i > i_lo) ? i_lo + 1 : i_lo;
            if (i_hi >= nr) {
                y = 0.0;
            }
            else if (i_hi == i_lo) {
                ...
                a = (data[i_hi]-data[i_lo])/sr;
                y = a*(r-i_lo*sr) + data[i_lo];
            }
            return y;
        }

        __device__ int double_to_idx(double x, double wf_nm, double pixel_nm) {
            return nearbyint((x + wf_nm / 2) / pixel_nm);
        }
        
        __global__ void device_gen_psf(double *mcdata, double *z_psf, double *location, uint16_t mat_size, double wf_nm, double pixel_nm, double sr, uint16_t nr)
        {
            int blockId = blockIdx.x + blockIdx.y * gridDim.x + gridDim.x * gridDim.y * blockIdx.z;
            int threadId = blockId * (blockDim.x * blockDim.y * blockDim.z) + (threadIdx.z * (blockDim.x * blockDim.y)) + (threadIdx.y * blockDim.x) + threadIdx.x;
            
            double val_x = -wf_nm / 2 + pixel_nm * (double)(threadId % mat_size);
            double val_y = -wf_nm / 2 + pixel_nm * (double)(threadId / mat_size);
            double r = sqrt(pow((val_x - location[0]),2) + pow((val_y - location[1]),2));
            
            z_psf[threadId] += location[2] * psf_interp1(r, sr, nr, mcdata);
        }
        
        __global__ void device_pattern_to_matrix(double *pattern, double *output, uint16_t slice, uint16_t mat_size, double wf_nm, double pixel_nm)
        {
            int blockId = blockIdx.x + blockIdx.y * gridDim.x + gridDim.x * gridDim.y * blockIdx.z;
            int threadId = blockId * (blockDim.x * blockDim.y * blockDim.z) + (threadIdx.z * (blockDim.x * blockDim.y)) + (threadIdx.y * blockDim.x) + threadIdx.x;
            //int threadId =  threadIdx.x + blockIdx.x * blockDim.x;
            
            int ix = double_to_idx(pattern[slice*threadId], wf_nm, pixel_nm);
            int iy = double_to_idx(pattern[slice*threadId+1], wf_nm, pixel_nm);
            
            unsigned int pos = ix + iy*mat_size;
            output[pos] += pattern[slice*threadId+2];
        }
        '''
        mod = SourceModule(src_module, arch="sm_50") #, arch="sm_50"
        kernel_gen_psf = mod.get_function("device_gen_psf")
        kernel_pattern_to_matrix = mod.get_function("device_pattern_to_matrix")
        
    def psfgen_gpu(self, psf_fct, location=[0.0, 0.0, 1.0], block=(32,16,1), grid=(128,64,1)):
        """[summary]

        Args:
            mcdata ([ndarray]): monte-carlo simulation data EDD(r)
        ...
        #block = (32, 16, 1)
        #grid = (self.mat_size//block[0], self.mat_size//block[1], 1)
        h_psf2d = np.zeros(self.mat_size**2, dtype=np.float64) #double precision,
        sr = psf_fct[1,0] - psf_fct[0,0]; nr = psf_fct.shape[0] #step in r and number of steps
        mcdata = np.array(psf_fct[:,1], dtype=np.float64)
        start = timer()
        kernel_gen_psf(
            cuda.In(mcdata),
            cuda.InOut(h_psf2d),
            cuda.In(np.array(location, dtype=np.float64)),
            np.uint16(self.mat_size),
            np.float64(self.wf_nm),
            np.float64(self.pixel_nm),
            np.float64(sr),
            np.uint16(nr),
            block=block,
            grid=grid
        ...
        log_message = f'min value = {np.min(h_psf2d)}; max value = {np.max(h_psf2d)}; sum value = {np.sum(h_psf2d)}.'
        print(log_message)
        return h_psf2d.reshape((self.mat_size, self.mat_size))
        
    def dosegen_gpu(self, pattern, block=(61,11,1), grid=(137,3,1)):
        """[summary]
        ...
        #block = (32, 16, 1) #!WARNING: to tweak
        #grid = (16, 32, 1)
        slice = pattern.shape[-1]
        h_dose2d = np.zeros(self.mat_size**2, dtype=np.float64) #double precision
        apattern = pattern.reshape(pattern.shape[0]*slice)
        start = timer()
        kernel_pattern_to_matrix(
            cuda.In(apattern),
            cuda.InOut(h_dose2d),
            np.uint16(slice),
            np.uint16(self.mat_size),
        ...
        log_message = f'overlap = {h_dose2d.sum()-pattern[:,2].sum()}. Negative value = lost dose.'
        print(log_message)
        return h_dose2d.reshape((self.mat_size, self.mat_size))
...
KERNEL_RADIUS = 8 #8 or 360 = max pixels
UNROLL_INNER_LOOP = True
KERNEL_W = 2 * KERNEL_RADIUS + 1
ROW_TILE_W = 256 #128=NP/KERNEL_RADIUS
KERNEL_RADIUS_ALIGNED = 16
COLUMN_TILE_W = 64 #16=1024/64
COLUMN_TILE_H = 64 #48=768/16
template = '''
...
def convolution_cuda(sourceImage,  filterx,  filtery):
    ...
    filterx = np.float32(filterx)
    filtery = np.float32(filtery)
    ...
    convolutionColumnGPU(destImage_gpu,  intermediateImage_gpu,  DATA_W,  DATA_H,  np.int32(COLUMN_TILE_W * threadBlockColumns[1]),  np.int32(DATA_W * threadBlockColumns[1]),  grid=grid_cols,  block=block_cols)

    # Pull the data back from the GPU.
    cuda.memcpy_dtoh(destImage,  destImage_gpu)
    return destImage
'''
z_expo=np.copy(your_binary_data)
z_expo=np.float32(z_expo)
filterx = gaussian_kernel(width=8) #8 for the edge, 64 for the background
target_exposure = z_expo.copy()
target_exposure [:] = np.nan
target_exposure = convolution_cuda(z_expo,  filterx,  filterx)
'''